...
app.get("/api/videos", async (req, res) => {
  try {
    const q = (req.query.q || "").trim();
    if (!q) return res.json({ query: q, results: [] });
    const embedUrl = `https://www.youtube.com/embed?listType=search&list=${encodeURIComponent(q)}`;
    res.json({ query: q, results: [embedUrl] });
  } catch (err) {
    console.error("Video error:", err);
    res.status(500).json({ error: "video_failed", details: String(err) });
  }
});
app.get("*", (req, res) => {
